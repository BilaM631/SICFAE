...
class CandidatoAdmin(SimpleHistoryAdmin):
    list_display = ('codigo_candidato', 'nome_completo', 'vaga', 'numero_bi', 'provincia', 'distrito', 'estado', 'data_criacao')
    list_filter = ('vaga', 'estado', 'provincia', 'distrito', 'data_criacao')
    search_fields = ('codigo_candidato', 'nome_completo', 'numero_bi', 'provincia__nome', 'distrito__nome')
    readonly_fields = ('codigo_candidato', 'data_criacao', 'data_atualizacao')
    actions = ['enviar_whatsapp_massivo']
    change_list_template = "admin/candidaturas/candidate/change_list.html" 
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return qs
        return obter_candidatos_acessiveis(request.user) 
    ...
    def enviar_whatsapp_massivo(self, request, queryset):
        from .services import ServicoWhatsApp 
        ...
        results = ServicoWhatsApp.enviar_mensagens_massa(queryset, msg_template) 
...
class EntrevistaAdmin(SimpleHistoryAdmin):
    list_display = ('candidato', 'data_hora', 'local', 'entrevistador', 'status', 'resultado', 'media_final')
    list_filter = ('status', 'resultado', 'data_hora', 'entrevistador')
    search_fields = ('candidato__nome_completo', 'candidato__numero_bi', 'local')
    autocomplete_fields = ['candidato', 'entrevistador']
    date_hierarchy = 'data_hora'
    
    fieldsets = (
        ('Agendamento', {
            'fields': ('candidato', 'data_hora', 'local', 'entrevistador', 'status')
        }),
        ('Avaliação', {
            'fields': ('nota_tecnica', 'nota_comunicacao', 'nota_experiencia', 'observacoes', 'resultado'),
            'classes': ('collapse',),
            'description': 'Preencher após a realização da entrevista.'
        }),
    )
